...
class RecoveryPointManager:
    ...
    def process_model_output(self, model_output: Dict[str, torch.Tensor]) -> Optional[RecoveryPoint]:
        ...
        """
        # Get path status probabilities
        path_probs = torch.sigmoid(model_output['path_status']).cpu().numpy()
        ...
        num_open = sum(open_directions)
        
        # If at least one direction is open, this is a recovery point
        if num_open > 0:
            point = RecoveryPoint(
                index=self.current_index,
                open_directions=open_directions.tolist(),
                rank=num_open,
                timestamp=self.timestamp,
                confidence=path_probs.tolist()
            )
    ...
    def is_dead_end(self, model_output: Dict[str, torch.Tensor]) -> bool:
        ...
        # Check if all directions are closed
        return all(prob <= self.confidence_threshold for prob in path_probs)
    ...
    def get_recovery_strategy(self, model_output: Dict[str, torch.Tensor]) -> dict:
        ...
        if self.is_dead_end(model_output):
            ...
            # Get confidence scores for current position
            path_probs = torch.sigmoid(model_output['path_status']).cpu().numpy()
            dead_end_prob = torch.sigmoid(model_output['is_dead_end']).cpu().numpy()
